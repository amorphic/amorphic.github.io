...
/usr/bin/pandoc -f markdown+smart -s --metadata title="James Anthony Stewart" $FILENAME.md -c $FILENAME.css -o $FILENAME.html
cp $FILENAME.html index.html

# docx 
echo "Building ${FILENAME}.docx"
/usr/bin/pandoc -f markdown+smart -s --metadata title="James Anthony Stewart" $FILENAME.md -c $FILENAME.css -o $FILENAME.docx
